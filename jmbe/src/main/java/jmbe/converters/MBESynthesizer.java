...
public abstract class MBESynthesizer
{
    ...
    {
        return mWhiteNoiseGenerator.getSamples(160, 0.01f);
    }
    ...
    public float[] getUnvoiced(MBEModelParameters parameters)
    {
        ...
        for(int l = 1; l <= parameters.getL(); l++)
        {
            if(!voicedBands[l])
            {
                ...
                //Algorithm documentation error - have to add 1 to make this an averaging calculation, otherwise
                //we get a divide by 0 error when a = b
                float denominator = (float)(b_max[l] - a_min[l] + 1);

                float scalor = UNVOICED_SCALING_COEFFICIENT * M[l] / (float)Math.sqrt((numerator / denominator));

                //Since the min/max for each L band can overlap, we'll select the larger scalor value
                for(int n = a_min[l]; n < b_max[l]; n++)
                {
                    if(n < 128 && scalor > dftBinScalor[n])
                    {
    ...
    public float[] getVoiced(MBEModelParameters currentFrame)
    {
        ...
        int threshold = (int)Math.floor((float)currentL / 4.0d);
        ...
        for(int n = 0; n < SAMPLES_PER_FRAME; n++)
        {
            for(int l = 1; l <= maxL; l++)
            {
                if(currentVoicing[l] && previousVoicing[l])
                {
                    ...
                    {
                        //Alg #133
                        voiced[n] += 2.0 * (synthesisWindow(n) * previousM[l] *
                ...
                {
                    //Alg #131
                    float value =  (synthesisWindow(n) * previousM[l] *
                ...
                {
                    //Alg #132
                    float value = (synthesisWindow(n - SAMPLES_PER_FRAME) * currentM[l] *
